...
import buildSceneUrl from "../../../../utils/buildSceneUrl";
...
function RoomIdea({
  ...
  const params = {
    roomId: param ? getFirstParam(param) : "",
  };

  const GetRoomScenesInfo: any = useQuery(GetRoomScenes, params);
  ...
  useEffect(() => {
    ...
    const selectedScene = sceneData.filter((scene: any) =>
      scene.url.includes(sceneParam)
    );
      ...
      {selectedScene ? (
        <>
          <RoomScene
            ...
            collectionId={selectedScene.collectionId}
          ...
          <GoBackButton
            ...
            url={`/shop-by-room${
              data
                ? data.filter((item: any) => item.name === param)[0]?.url
                : ""
            }`}
          />
        </>
        ...
        <div data-fs-room-ideas-grid-container>
          <div data-fs-room-ideas-grid-items>
            {sceneList &&
              sceneList.map((sceneItem: any, index: number) => (
                <a
                  key={index}
                ...
                >
                  <img
                    data-fs-grid-item-img
                    data-fs-grid-scene
                    data-fs-grid-idea="true"
                    src={sceneItem.image}
                    alt={sceneItem.alt}
                  />
                  <Button data-fs-grid-item-btn>{"SHOP NOW"}</Button>
                </a>
              ))}
          </div>
          <GoBackButton param={"Room"} />
        </div>
      ) : (
        <div data-fs-room-ideas-grid-items>
          {data &&
            data.map((gridItem: any, index: number) => (
              <a
                key={index}
              ...
              >
                <img
                  data-fs-grid-item-img
                  data-fs-grid-idea={gridItem.button ? true : false}
                  src={gridItem.image}
                  alt={gridItem.alt}
                />
